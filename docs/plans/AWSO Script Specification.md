...
Every `awso restore` must regenerate `AGENTS.override.md`. An existing regular
file or symlink is replaced regardless of its contents, generation marker, or
Git tracking state. This is the explicit exception to the static-overlay
destination safety rules above. A directory or unsupported filesystem entry at
that path remains an error.
...
10. Check whether `AGENTS.override.md` exists and has the expected content and file type.
...
AWSO must never overwrite or shadow these paths when restoring static overlay
files. The sole exception is `AGENTS.override.md`, which `awso restore` always
regenerates even if Git tracks it.
