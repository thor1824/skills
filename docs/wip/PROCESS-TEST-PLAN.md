...
- Missing handoff fields: `type`, mapped issue `status`, `category`, `blocked_by`
- Wrong status semantics between PRDs and Issues
- `/triage` accepting invalid state transition
- `/to-prd` writing an unapproved PRD file
- `/to-issues` slicing a closed PRD
...
| prd generation | prepared repo + feature request | minimal valid context | presents a PRD draft in chat, writes `.scratch/<slug>/PRD.md` only after approval, and uses only `type: PRD` in initial front matter | `.scratch` write only after approval | no unapproved PRD file |
...
| issue slicing open prd | PRD with no `status` | approved boundary | creates issue files with `type: Issue`, `category`, `blocked_by`, mapped `needs-triage` status | `.scratch` writes | direct handoff to triage |
| issue slicing closed prd | PRD with `status: done` or `status: wontfix` | closed boundary | refuses to publish issues from the PRD | no invalid issue creation | critical governance check |
| triage PRD close path | PRD with no `status` | `type: PRD` | only closes to `done` or `wontfix` on explicit maintainer direction | note write if requested | PRD must never go to `ready-for-agent` |
...
| end-to-end golden path | feature request in blank repo | full chain | `/prepare-repo` -> PRD approval/write -> issues -> triage `ready-for-agent`/human with correct artifacts | all expected writes | main regression test |
...
Run `/prepare-repo`, `/to-prd`, `/to-issues`, `/triage`.
...
### 3. Closed PRD blocked from slicing

Feed `/to-issues` a PRD with `status: done` or `status: wontfix`.

Assert no issue files are produced and the response explains that closed PRDs cannot be sliced.
