...
If `CONTEXT-MAP.md` exists, read it before updating domain docs. Infer which context the current topic belongs to. If the topic could belong to multiple contexts and the choice affects terminology or ADR location, ask which context to use.
...
At the end of a grilling session, or before returning control to another skill, summarize the output in durable form:
...
If the session is running inside `/triage`, make sure these facts can be copied into the issue's triage notes or agent brief.
